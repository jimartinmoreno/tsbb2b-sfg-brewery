...
public class BeerOrderServiceImpl implements BeerOrderService {
    ...
    private BeerOrder getOrder(UUID customerId, UUID orderId) {
        System.out.println("getOrder - customerId = " + customerId);
        System.out.println("getOrder - orderId = " + orderId);
        Optional<Customer> customerOptional = customerRepository.findById(customerId);

        System.out.println("getOrder - customerOptional.isPresent = " + customerOptional.isPresent());

        if (customerOptional.isPresent()) {
            Optional<BeerOrder> beerOrderOptional = beerOrderRepository.findById(orderId);
            System.out.println("getOrder - beerOrderOptional.isPresent = " + beerOrderOptional.isPresent());

            if (beerOrderOptional.isPresent()) {
                BeerOrder beerOrder = beerOrderOptional.get();

                System.out.println(beerOrder.getCustomer().getId() + " = " + customerId);
                // fall to exception if customer id's do not match - order not for customer
                if (beerOrder.getCustomer().getId().equals(customerId)) {
                    System.out.println("beerOrder = " + beerOrder);
                    return beerOrder;
                }
            }
